...
const RoundTable = ({
  ...
  const getNextVideo = () => {
    if (currentVideoIndex < videos?.length - 1) {
      setCurrentVideoIndex((old) => {
        changeUrl({ index: old + 1 })
        return old + 1
      })
    } else {
      setCurrentVideoIndex(old => {
  ...
  const changeUrl = ({ index = 0, setLoopPage = false }) => {
    if (setLoopPage) {
      window.history.replaceState(null, '', `/l/${details.share_string}`)
      return
    }
    window.history.replaceState(null, '', `/v/${videos?.[index]?.share_string}?l=${details?.share_string}`)
  }
        ...
        <Layout>
          ...
          {isOpen &&
            <Player
              ...
              videoId={videos?.[currentVideoIndex]?.share_string}
            ...
            >
              {!mobile && (
                <Box
                  ...
                  onClick={() => {
                    onOpen()
                    window.history.pushState(null, '', `/v/${video.share_string}?l=${chatId}`)
