# AI Context — Project Patterns & State

<!-- Read this on demand when working on React/Next.js, Tailwind v4, env vars, or package-specific
     work. It is NOT auto-injected — load it when the task touches these domains. -->
...
| Native Claude Code (no sync, no router)   | `.claude/` is authored directly; token-efficient |
...
**Date:** 2026-05-30
**What changed:** Relocated from `.team/docs/` to `.claude/docs/` as part of the go-native AI-config
migration. Dropped stale TODO sections (Current focus, Known issues) and the obsolete sync/`.team`
operational notes. Reference content (Tailwind v4, React/Next.js, packages, env vars) kept intact.
