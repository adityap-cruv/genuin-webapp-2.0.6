        ...
        <nav className="sticky top-0 flex h-[76px] w-full items-center justify-between px-2 xl:container">
          ...
          {!isEmbed && (
            <div className="flex gap-x-3">
              ...
              {user.status === 'unauthenticated' && <Button onClick={AuthenticationModal.open}>Log in</Button>}
              {/* {user.status === 'authenticated' && (
                <Popover>
                  <PopoverTrigger>
                    <div className="flex items-center rounded-full border border-monochrome-9 p-1">
                      <div className="h-7 w-7 rounded-full bg-blue-10"></div>
                      <svg
                  ...
                  <PopoverContent
                    ...
                    <div className="my-2 flex items-center gap-2">
                      <div className="h-12 w-12 rounded-full bg-blue-10"></div>
                      <div>
                        <p className="text-title-3-bold">{user.data.user.email}</p>
