...
export function GeneralShell({ titleNode, subtitle, animationUrl }: GeneralShellProps) {
  return (
    <AnimatedTile className="py-9 md:py-14">
      <div className="flex flex-col gap-4 md:gap-6">
        <p className="text-[36px] font-bold leading-none md:w-3/4 md:text-[56px]">{titleNode}</p>
        <p className="text-cap-1-bold-home font-medium md:w-3/4 md:text-title-1-med">{subtitle}</p>
      </div>
      <VanillaPlayer
        style={{ borderRadius: 36 }}
        className="mx-5 aspect-square w-full max-w-sm overflow-clip object-fill md:mx-0 md:w-1/2 lg:w-1/3 lg:max-w-none"
