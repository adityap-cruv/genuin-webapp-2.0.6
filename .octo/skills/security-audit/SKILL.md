---
name: security-audit
description: Read-only audit of code for security vulnerabilities and review of auth/input/secrets changes — injection, XSS, dangerouslySetInnerHTML, auth-token storage, CORS/CSP, exposed secrets, unsafe external data at trust boundaries. Use when asked whether code is safe, to review an auth change, or to hunt for vulnerabilities. Do NOT modify files, and never change auth/CORS/CSP without explicit team approval.
---

# Security Audit

**When to use:** Auditing code for security vulnerabilities, reviewing auth changes, hunting for vulnerabilities, or checking that a feature meets the project's security requirements — e.g. when asked whether code is safe or to review an auth/input/secrets change.

**Do NOT use for:** Modifying files. This skill is **read-only** — it identifies and reports issues only. Never change auth, CORS, CSP, or security headers without explicit team approval.
