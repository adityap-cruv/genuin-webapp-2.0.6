...
**When to use:** when asked to "review my UI", "check the design/UX", "audit the visuals", or polish a component in this React 19 + Next 15 + Tailwind v4 stack. Covers *design, UX, motion, typography, perf-CLS, i18n, theming*.

**Do NOT use for:** accessibility/WCAG audits (semantic HTML, focus management, form labels, contrast, ARIA, keyboard) — those are owned by the **`accessibility`** skill; run that for a11y. This skill assumes a11y is handled and does not duplicate it. Also do NOT use for React/data architecture (use **`frontend-patterns`**).
...
Use `path:line - issue`. Sacrifice grammar for brevity. No preamble or explanation unless the fix is non-obvious. Don't restate code back.
...
### Hydration Safety
- No `Date.now()`/`Math.random()`/locale-dependent output in render without `suppressHydrationWarning` or a client-only guard — causes hydration mismatch.
...
> Source: adapted from Vercel Labs `web-design-guidelines` (Web Interface Guidelines). Vendored as a local checklist (no runtime WebFetch) to keep reviews offline and deterministic.
