...
**When to use:** Auditing, writing, or fixing code for accessibility — reviewing or improving a11y, keyboard support, screen-reader semantics, focus management, form labels, colour contrast, ARIA, or reduced-motion. Target standard is WCAG 2.1 Level AA.

**Do NOT use for:** General UI building (use frontend-patterns) or non-a11y visual/design review.
...
- [ ] Interactive elements use native HTML: `<button>` for actions, `<a href>` for navigation — never `<div onClick>` or `<span onClick>`
...
- [ ] Loading states announced to screen readers via `aria-live="polite"` or `aria-busy="true"`
...
- [ ] Custom widgets implement the correct ARIA pattern from APG (https://www.w3.org/WAI/ARIA/apg/patterns/)
