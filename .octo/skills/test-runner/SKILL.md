...
**When to use:** When asked to run, fix, or write unit/integration tests in this project — invoking the correct test command, interpreting failures, writing colocated `*.test.ts` files, mocking, async assertions, and closing coverage gaps.

**Do NOT use for:** Playwright end-to-end tests (use the e2e-testing skill) or debugging non-test runtime bugs (use the debug skill).
...
**`packages/ui` runs both Jest and Vitest** (`jest && vitest run`) because Jest is needed for component snapshot tests alongside Vitest for unit tests. Do not remove Jest from `packages/ui`.
...
2. **Fixing failing tests** — read the failure message, find the cause, fix the implementation or the test (not both at once).
