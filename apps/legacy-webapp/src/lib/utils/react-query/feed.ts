...
import { reverseMapCommunityUserRole, type CommunityUserRole } from '@/components/tree-structure'
import { type InfiniteData, type QueryKey } from '@tanstack/react-query'
import { type FetchFeedReturnType } from './type'
 ...
 * @param newRole
 */
export const updateCommunityJoinStatus = (queryKey: QueryKey, communityId: string, newRole: CommunityUserRole) => {
  queryClient.setQueryData<QueryDataType>(queryKey, (oldData): QueryDataType | undefined => {
 ...
 * @param isSparked
 */
export const updateSparkStatus = (queryKey: QueryKey, videoId: string, isSparked: boolean) => {
  queryClient.setQueryData<QueryDataType>(queryKey, (oldData): QueryDataType | undefined => {
    if (!oldData) return oldData
    return {
      ...oldData,
            ...
            if (video.uuid === videoId) {
              return {
                ...
                video: {
                  ...
                  no_of_sparks: isSparked ? video.video.no_of_sparks + 1 : video.video.no_of_sparks - 1,
                },
              }
            }
            return video
 ...
 * @param count
 */
export const updateCommentCount = (queryKey: QueryKey, videoId: string, count: number) => {
  queryClient.setQueryData<QueryDataType>(queryKey, (oldData): QueryDataType | undefined => {
    if (!oldData) return oldData
    return {
      ...oldData,
