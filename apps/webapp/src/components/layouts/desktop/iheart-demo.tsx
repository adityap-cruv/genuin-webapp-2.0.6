...
function AudioPlayer({ inModal, ...restProps }: AudioPlayerPropsType) {
  ...
  useEffect(() => {
    ...
    if (communityIndex !== -1) {
      const audioUrl = audioUrls[communityIndex];
      if (audioUrl && iframe.src !== audioUrl) iframe.src = audioUrl;
    }
  }, [pathName]);
    ...
    <>
      <div
        ...
        <section
          ...
          className={cn(
            "border-new-off-black/40 relative m-auto flex w-full items-center justify-between gap-2 overflow-clip 2xl:container 2xl:px-0"
          )}
          style={{ height: "75px" }}>
          <iframe
            src={audioUrl}
          ...
          {!isPipOpen && !isMobile && !inModal && !isFullScreen && (
            <div onClick={handleOpenPipClick} className="bg-tertiary-200 cursor-pointer rounded-lg p-2">
              <PipIcon />
            </div>
          )}
        </section>
