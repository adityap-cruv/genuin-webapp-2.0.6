...
export async function middleware(request: NextRequest) {
  ...
  if (request.nextUrl.pathname === "/home") {
    ...
    const hasOAuthParams = request.nextUrl.searchParams.has("code") || request.nextUrl.searchParams.has("state");

    // Only redirect if coming from Google (but not OAuth) and no auth-related params
    if (referer && !hasOAuthParams && referer.includes("google.com") && !referer.includes("accounts.google.com")) {
      return NextResponse.redirect("https://begenuin.com");
    }
  }
...
export function getConfig(host: string): { domain?: string; subdomain?: string } | null {
  // Strip 'www.' prefix if present to normalize the host
  let normalizedHost = host;
  if (host.startsWith("www.")) {
    normalizedHost = host.substring(4);
  }

  // Special cases for localhost and other development environments
  const firstSegment = normalizedHost.split(".")[0] ?? "";
  if (["localhost:4005", "localhost:4000", "localhost:3000", "192"].includes(firstSegment)) {
    return { subdomain: "app" };
  }

  // External domains
  if (!normalizedHost.includes("begenuin")) {
  ...
  const subdomain = isDevEnvironment
    ? normalizedHost.replace(".qa.begenuin.com", "").replace(".uat.begenuin.com", "")
    : normalizedHost.replace(".test.begenuin.com", "").replace(".begenuin.com", "");
