/**
 * @fileoverview Feature: SDK Visual Regression — TED layout.
 * Captures a baseline screenshot of the TED embed and diffs against the on-disk baseline.
...
test.describe('Feature: SDK Visual Regression — TED Layout', () => {
  let sharedPage: Page
  ...
  test('Scenario: SDK renders correctly for ted layout', async () => {
    const layoutData = EMBED_LAYOUT_DATA.ted

    await test.step('Setup: Create SDK container with dimensions for ted', async () => {
    ...
    await test.step('Action: Initialize SDK with ted embed_id and api_key', async () => {
    ...
    await test.step('Result: Visual snapshot matches baseline', async () => {
      ...
      await expect(container).toHaveScreenshot('ted.png', { animations: 'disabled' })
    })
  })
})
