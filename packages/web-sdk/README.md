...
End-to-end tests are written using [Playwright](https://playwright.dev/) and is currently only checking ui rendering.

### Prerequisites

Before running tests, build SDK:
...
> Tests load `dist/gen_sdk.js` via `index-test.html`. This file is auto-created by the global setup if missing, if you already have one then it wont overwrite it.
...
npm run test:headed
 # Run in debug mode(step through each line)
npm run test:debug
...
web-sdk
   └── tests/
       ├── e2e/
       │   └── init/                    # All E2E spec files
       ├── helpers/
       │   ├── page.helper.ts           # Load test page
       │   ├── sdk-container.helper.ts  # Create/manage SDK containers
       │   ├── sdk-init.helper.ts       # SDK initialization methods
       │   ├── sdk-wait.helper.ts       # Wait/assertion utilities
       │   └── cleanup.helper.ts        # State cleanup between tests
       ├── data/
       │   └── init.ts                  # Test data & config constants
       ├── utils/
       │   └── type.ts                  # Shared test types
       ├── global-setup.ts              # Creates index-test.html if missing
       ├── test-results/                # Screenshots, traces & failure artifacts
       ├── results
             ├── playwright-report/           # HTML test report
             └── test-docs/                   # test2doc generated documentation
```
...
> Screenshots and traces for failed tests are saved in `tests/results/test-results/`
...
4. **Inspect the trace** — Playwright captures a trace on failure. Open it with:
   ...
   ```bash
   npx playwright show-trace tests/results/test-results/<test-folder>/trace.zip
   ```
...
6. **Common failure checklist**:
   - [ ] Is `dist/gen_sdk.js` built? Run `npm run build`
   - [ ] Are `.env.development` values (`embed_id`, `api_key`) valid?
   - [ ] Are Playwright browsers installed? Run `npx playwright install`
   - [ ] Is the `index-test.html` file present in the root?

### FAQ

**Q: What is `index-test.html`?**
A minimal HTML page that loads `dist/gen_sdk.js`. It is auto-generated by `global-setup.ts` if it doesn't exist.

**Q: Tests fail with "video not found"?**
Ensure your `.env.development` has valid `embed_id` and `api_key` with real content. Check `tests/data/init.ts`.

**Q: `index-test.html` not found error?**
Run `npm run build` first so `dist/gen_sdk.js` exists before the test page is served.

**Q: Shadow DOM tests fail?**
Verify `useShadowDOM: true` is passed in `init()` and the SDK version supports it.

**Q: How are tests isolated between runs?**
Each test suite uses `cleanupSDKState` or `cleanupMultiEmbedState` in `afterEach` which reloads the page and recreates containers.

**Q: Where are screenshots and failure artifacts stored?**
All failure artifacts (screenshots, videos, traces) are stored in `tests/test-results/`.
