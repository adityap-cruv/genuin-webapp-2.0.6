...
import { cn, getWebpUrlForImage } from '@/utils'
...
>(({ value, className, spriteUrl, ...props }, ref) => {
  const { timeState, showScrubber, showSeeker } = usePlayerContext()
  const scrubberStartTime = React.useMemo(
    () => ((value?.[0] ?? 0) / 100) * timeState.duration,
    [value, timeState.duration],
  )

  const time = React.useMemo(() => {
    if (!timeState.duration || !value?.[0]) return '00:00'

    const totalSeconds = (value[0] / 100) * timeState.duration
  ...
  return (
    <SliderPrimitive.Root
      ...
      <SliderPrimitive.Track
        className={cn(
          'relative h-[3px] w-full grow overflow-hidden bg-secondary transition-all',
        )}>
        ...
        <SliderPrimitive.Thumb
          ...
          {showScrubber && (
            <div
              ...
              <p className='text-title-3-demi text-white'>{time}</p>
            </div>
          )}
        </SliderPrimitive.Thumb>
      )}
...
ScrubberSlider.displayName = SliderPrimitive.Root.displayName
