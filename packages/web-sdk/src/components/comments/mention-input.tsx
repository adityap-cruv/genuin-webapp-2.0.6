...
const MentionInput: React.FC<{
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (urlMatches) {
      urlMatches.forEach((url: any) => {
        const isUrlAlreadyAdded = selectedMentions.some(
          (mention: any) => mention.id === url,
        )
        if (!isUrlAlreadyAdded) {
          setSelectedMentions((prev: any) => [
            ...prev,
            { handle: url, id: url, type: 'url' },
          ])
        }
      })
    }
  ...
  const convertCommentTextToArray = (
    commentText: string,
  ...
  return (
    <div>
      {isMentioning && (
        <div
          ref={mentionListRef}
          className='absolute bottom-16 left-0 z-50 flex max-h-60 w-full flex-col gap-1 overflow-hidden overflow-y-scroll rounded-t-2xl bg-background p-3'
            ...
            filteredMentions.map((mention: any) => {
              ...
              return (
                <div
                  key={
                    isCommunity
                      ? mention.community.community_id
                      : mention.user.name
                  }
                  onClick={() => {
                    handleUserSelect(mention)
                  }}
                  className='hover:bg-tertiary-200 flex flex-1 cursor-pointer items-center gap-x-3 rounded-md p-2'>
                  <CustomAvatar
                  ...
                  />
                  <div className='pr-2'>
                    <p className='line-clamp-1 break-all text-title-3-bold'>
                      {name}
                    </p>
                    <p className='text-gray-500 line-clamp-1 break-all text-body-1-med'>
                      {description}
                    </p>
                  </div>
                </div>
        ...
        <div className='flex w-full flex-1 items-center gap-x-4 px-4'>
          {user ? (
            <div className='w-full'>
              <div className='flex items-center rounded-full border border-tertiary-300 bg-tertiary-100 px-4'>
                <Input
                  ref={textareaRef}
                  ...
                  maxLength={500}
                  disabled={!user}
                  className='border-none rounded-full bg-none'
