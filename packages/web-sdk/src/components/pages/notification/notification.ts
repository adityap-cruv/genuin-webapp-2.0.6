...
async function fetchNotifications(limit: number, pageParam?: { first_notification_id?: string; last_notification_id?: string }) {
  try {
    const url = new URL(getApiUrl('/api/v3/notifications'))
    url.searchParams.append('limit', limit.toString())
    
    if (pageParam?.last_notification_id) {
      url.searchParams.append('last_notification_id', pageParam.last_notification_id)
    }
    ...
    return { notifications: resData.data.notifications, end: !!resData.data.end_of_notifications }
  } catch (error) {
...
export function getNotifications(limit: number) {
  return useInfiniteQuery({
    queryFn: async ({ pageParam }: { pageParam?: { first_notification_id?: string; last_notification_id?: string } }) => await fetchNotifications(limit, pageParam),
    queryKey: ['notification', 'paginated'],
    getNextPageParam: (lastPage: { notifications: { notification_id: string }[]; end: boolean }) => {
      if (lastPage.end) {
        return undefined
      }
      return {
