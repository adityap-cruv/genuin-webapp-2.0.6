...
async function fetchLoopVideo(loopId: string, videoId: string): Promise<LoopVideoType> {
...
async function getVideoDetails(slug: string): Promise<FeedVideoType> {
  ...
  return {
    ...
    community: {
      ...
      logged_in_user_role: loopDetails.community.logged_in_user_role ?? undefined,
      is_join_requested: false,
      brand: {
        brand_id: loopDetails.community.brand?.brand_id ?? 0,
        brand_slug: loopDetails.community.brand?.brand_slug ?? '',
        brand_user_logo: typeof loopDetails.community.brand?.logo === 'string' ? parseInt(loopDetails.community.brand.logo, 10) : loopDetails.community.brand?.logo ?? 0,
    ...
    video: {
      ...
      linkouts: null,
      linkouts_id: videoDetails.linkouts_id ? String(videoDetails.linkouts_id) : '',
      ...
      meta_data: {
        ...
        contains_external_videos: videoDetails.meta_data?.contains_external_videos ?? false,
