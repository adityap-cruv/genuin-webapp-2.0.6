...
import { NOT_FOUND_ERROR_MESSAGES } from '@/utils/constants/errors'
// import { TopBar } from '../layouts/mobile/top-bar'
import { HeaderMobile } from './header/mobile'
// import { NoContentIcon } from '@images/not-found/no-content-icon'
import { NoContentIcon } from './icons/no-content-icon'
...
export function NotFoundView({ type }: Props) {
    ...
    return (
      <main className='h-full w-full'>
        <div className='block sm:hidden'>
          <HeaderMobile variant={'transparent'} />
        </div>
        <div className='h-full w-full p-4'>
          <div
            ...
            <p className='text-title-2-demi font-medium text-secondary-300'>
              No Content Available
            </p>
            {/** TODO: check with krunal bhai regarding this button */}
            {/* {showCreateCommunityButton && (
              <Link
                href={process.env.NEXT_PUBLIC_BCC_URL + '/manage/communities'}>
                <Button
                  size='custom'
                  className='rounded-lg bg-new-off-black px-4 py-3 hover:bg-new-dark-grey'>
    ...
    <main className='h-full w-full'>
      ...
      <div className='h-full w-full p-4'>
        <div
          ...
          <img
            alt='puppet'
            src={getIconLink('puppet')}
            className='h-auto w-32'
          />
          {NOT_FOUND_ERROR_MESSAGES[type].showButton && (
            <CustomLink href={BCC_URL + '/manage/communities'}>
              <Button
                size='custom'
                className='rounded-lg bg-new-off-black px-4 py-3 hover:bg-new-dark-grey'>
