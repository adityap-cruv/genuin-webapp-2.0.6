...
export default function PlaybackSpeedControlSlider() {
  ...
  return (
    <div className='bg-white/80 rounded-2xl p-6 pt-0 backdrop-blur-md'>
        ...
        <Slider
          ...
          onValueChange={(val) => {
            const closestSpeed = PLAYBACK_SPEEDS.reduce((prev, curr) => {
              return Math.abs(curr - val[0]) < Math.abs(prev - val[0])
                ? curr
                : prev
            })
      ...
      <div className='mt-6 flex justify-between gap-4'>
        {PLAYBACK_SPEEDS.map((s) => (
          <Button
            ...
            className={`w-16 rounded-full px-4 py-2 text-title-3-demi hover:text-white sm:hover:bg-tertiary-400 ${
              playbackSpeed.speed === s
                ? 'bg-primary text-white'
                : 'text-black bg-tertiary-200'
            }`}>
            {s}
          </Button>
        ))}
