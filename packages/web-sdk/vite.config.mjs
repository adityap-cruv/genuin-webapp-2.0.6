...
const genuinResolver = () => ({
  ...
  resolveId(id, importer) {
    ...
    if (id === "@genuin/tailwind-config") {
      const configPath = resolve(__dirname, "../tailwind-config/shared-styles.css");
...
const copyLoaderPlugin = () => ({
  ...
  async writeBundle(options, bundle) {
    ...
    const loaderEntry = Object.keys(bundle).find((key) => key.startsWith("gen_sdk") && bundle[key].type === "chunk");
    ...
    if (loaderEntry) {
      ...
      const mediaBaseUrl =
        process.env.MEDIA_BASE_URL || process.env.NEXT_PUBLIC_MEDIA_BASE_URL || "https://media.qa.begenuin.com";
      ...
      loaderContent = loaderContent.replace(/__DEV_ENVIRONMENT__/g, nodeEnvCheck);
      ...
      if (sdkVersionPath && sdkVersionPath.trim()) {
        ...
        loaderContent = loaderContent.replace(/__SDK_VERSION_PATH__/g, versionPath);
      } else {
      ...
      try {
        ...
        cssFile = assetFiles.find((fileName) => fileName.startsWith("web-sdk") && fileName.endsWith(".css"));
        console.log("🔍 Found CSS file:", cssFile);
      } catch (error) {
        console.error("❌ Could not read dist directory:", error);
      }
      if (!sdkFile) {
        console.error("❌ Could not find hashed ES module file in dist directory!");
      ...
      if (sdkFile) {
        ...
        loaderContent = loaderContent.replace(/__SDK_FILENAME_PLACEHOLDER__/g, sdkFile);
      ...
      if (cssFile) {
        ...
        loaderContent = loaderContent.replace(/__CSS_FILENAME_PLACEHOLDER__/g, cssFile);
        ...
        loaderContent = loaderContent.replace(/__CSS_FILENAME_PLACEHOLDER__/g, "web-sdk.css");
      }
...
export default defineConfig({
      ...
      output: {
        ...
        assetFileNames: (assetInfo) => {
          ...
          if (assetInfo.name && assetInfo.name.endsWith(".css")) {
            ...
            return isProduction ? "assets/web-sdk-[hash].css" : "assets/web-sdk.css";
          }
  ...
  define: {
    ...
    // Debug logging
    __DEBUG_API_URL: JSON.stringify(process.env.NEXT_PUBLIC_API_URL || "NOT_FOUND"),
    ...
    "process.env.DOMAIN": JSON.stringify(process.env.DOMAIN || "qa.begenuin.com"),
    "process.env.BASE_URL": JSON.stringify(process.env.BASE_URL || "https://app.qa.begenuin.com"),
    "process.env.API_BASE_URL": JSON.stringify(process.env.API_BASE_URL || "https://api.qa.begenuin.com"),
    "process.env.MEDIA_BASE_URL": JSON.stringify(process.env.MEDIA_BASE_URL || "https://media.qa.begenuin.com"),
    "process.env.RUDDERSTACK_URL": JSON.stringify(process.env.RUDDERSTACK_URL || "https://rudderstack.qa.begenuin.com"),
    "process.env.RUDDERSTACK_API_KEY": JSON.stringify(process.env.RUDDERSTACK_API_KEY || ""),
    "process.env.ENCRYPTION_SALT": JSON.stringify(process.env.ENCRYPTION_SALT || ""),
    "process.env.ACCESS_TOKEN_KEY": JSON.stringify(process.env.ACCESS_TOKEN_KEY || ""),
    "process.env.UNIQUE_USER_ID_KEY": JSON.stringify(process.env.UNIQUE_USER_ID_KEY || ""),
    "process.env.ENCRYPTION_IV": JSON.stringify(process.env.ENCRYPTION_IV || ""),
    ...
    "process.env.TRACK_OBSERVABILITY": JSON.stringify(process.env.TRACK_OBSERVABILITY || "false"),
    ...
    "process.env.NEXT_PUBLIC_RUDDERSTACK_KEY": JSON.stringify(
      process.env.NEXT_PUBLIC_RUDDERSTACK_KEY || process.env.RUDDERSTACK_API_KEY || ""
    ),
    ...
    "process.env.NEXT_PUBLIC_HOST_URL": JSON.stringify(
      process.env.NEXT_PUBLIC_HOST_URL || process.env.BASE_URL || "https://app.qa.begenuin.com"
    ),
    "process.env.NEXT_PUBLIC_API_URL": JSON.stringify(
      process.env.NEXT_PUBLIC_API_URL || process.env.API_BASE_URL || "https://api.qa.begenuin.com"
    ),
    "process.env.NEXT_PUBLIC_AES_IV": JSON.stringify(process.env.NEXT_PUBLIC_AES_IV || process.env.ENCRYPTION_IV || ""),
    ...
    "process.env.NEXT_PUBLIC_REDIRECT_URI": JSON.stringify(
      process.env.NEXT_PUBLIC_REDIRECT_URI || process.env.BASE_URL || "https://app.qa.begenuin.com"
    ),
    ...
    "import.meta.env.TRACK_OBSERVABILITY": JSON.stringify(process.env.TRACK_OBSERVABILITY || "false"),
