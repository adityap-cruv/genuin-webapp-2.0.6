...
if (bundleSizeKB > 3000) {
  console.warn(
    `⚠️ Bundle is quite large (${bundleSizeKB} KB). Consider optimizations.`,
  )
...
if (bundleContent.includes('require(')) {
  console.log(
    '✅ CommonJS require calls found (normal for bundled dependencies)',
  )
}

if (
  bundleContent.includes('use client') ||
  bundleContent.includes('use server')
) {
...
if (bundleContent.includes('.css') && !bundleContent.includes('style')) {
  ...
  bundleContent.includes('style') ||
  bundleContent.includes('stylesheet')
) {
