...
function generateRecommendations(report: BaselineReport): string[] {
  ...
  if (report.chunksOverLimit.length > 0) {
    recommendations.push(`⚠️ Found ${report.chunksOverLimit.length} chunk(s) over 500KB limit:`);
    report.chunksOverLimit.forEach((chunk) => {
      recommendations.push(`  - ${chunk.name}: ${chunk.sizeMB.toFixed(2)}MB (should be split)`);
    });
  }

  // Check for undersized chunks
  if (report.chunksUnderLimit.length > 0) {
    recommendations.push(`⚠️ Found ${report.chunksUnderLimit.length} chunk(s) under 20KB (consider merging):`);
  ...
  if (indexChunk && indexChunk.size > 200 * 1024) {
    recommendations.push(`⚠️ Index chunk is ${indexChunk.sizeMB.toFixed(2)}MB (target: <200KB). Consider:`);
...
async function generateReport(): Promise<BaselineReport> {
  ...
  const averageChunkSize = chunks.length > 0 ? chunks.reduce((sum, c) => sum + c.size, 0) / chunks.length : 0;
...
async function main() {
  try {
    ...
    await import("fs/promises").then((fs) => fs.writeFile(reportPath, formatted, "utf-8"));
