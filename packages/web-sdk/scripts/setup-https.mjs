...
if (hasMkcert) {
  ...
  console.log(
    chalk.gray('   Certificates support localhost and network access.'),
  )
  process.exit(0)
}
...
if (setupMethod === 'http') {
  ...
  console.log(
    chalk.cyan('   npm run serve:sdk:network           # HTTP on your network'),
  )
  console.log(
    chalk.cyan('   npm run serve:sdk:qa:network        # QA HTTP on network'),
  )
  console.log(
    chalk.cyan('   npm run serve:sdk:prod:network      # Prod HTTP on network'),
  )
...
if (!mkcertInstalled) {
  ...
  if (platform === 'darwin') {
    ...
    try {
      ...
      console.log(
        chalk.red('❌ Homebrew not found. Please install Homebrew first:'),
      )
      console.log(
        chalk.cyan(
    ...
    console.log(
      chalk.red('❌ Unsupported platform. Please install mkcert manually:'),
    )
    console.log(
      chalk.cyan('   https://github.com/FiloSottile/mkcert#installation'),
    )
    process.exit(1)
  }
  ...
  if (!shouldInstall) {
    console.log(
      chalk.yellow(
        '⏭️  Skipping installation. Please install mkcert manually.',
      ),
    )
    process.exit(1)
  }
  ...
  try {
    ...
    console.log(
      chalk.yellow(
        '   Please install manually: https://github.com/FiloSottile/mkcert#installation',
      ),
    )
    process.exit(1)
  }
...
try {
  const networkInterfaces = execSync(
...
try {
  ...
  console.log(
    chalk.yellow('⚠️  CA installation may have failed. Continuing anyway...'),
  )
}
...
console.log(
  chalk.cyan('   npm run serve:sdk:https             # Development HTTPS'),
)
console.log(chalk.cyan('   npm run serve:sdk:qa:https          # QA HTTPS'))
console.log(
  chalk.cyan('   npm run serve:sdk:prod:https        # Production HTTPS'),
)
...
console.log(
  chalk.gray("   so you won't see security warnings in your browser."),
)
