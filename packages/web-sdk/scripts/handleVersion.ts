...
export function commitVersion() {
  ...
  try {
    const tempVersionInfo = JSON.parse(fs.readFileSync(TEMP_VERSION_FILE, "utf-8"));
    ...
    if (fs.existsSync(envFile)) {
      ...
      envContent = envContent.replace(/SDK_VERSION=.*$/m, `SDK_VERSION="${tempVersionInfo.version}"`);
...
async function promptVersion() {
  ...
  const versionInfo: VersionInfo = JSON.parse(fs.readFileSync(versionPath, "utf-8"));
  ...
  console.log(`\x1b[36mℹ Changes will be committed after successful build\x1b[0m\n`);
