 ...
 *
 * The mute icon starts as an enticement ONLY when the unit loads
 * unmuted-but-silent (`initialVolume === 0`, so `isMuted` is genuinely `true`):
 * the icon shows "sound on" to nudge the user to tap for audio. That enticement
 * must end only when the user does something *audio-related* — taps the
 * mute/unmute control, or unmutes via the ad tap-overlay (both surface as a
 * `mute:unmuted` bus event). When the tag starts audible (`initialVolume > 0`)
 * there is nothing to entice, so this hook reports engaged from the start.
 *
...
export function useAudioEngaged(locallyEngaged = false): boolean {
  ...
  // The enticement only exists for the silent-start case (initialVolume === 0):
  // a unit that loads at volume 0 shows "sound on" to nudge a tap. When the tag
  // is configured to start audible (initialVolume > 0) there is nothing to
  // entice — playback already has sound — so treat audio as engaged from the
  // start and let the mute icon track the real `isMuted` immediately.
  const { initialVolume } = useStrategy();
  const audibleStart = initialVolume > 0;
  ...
  const [engaged, setEngaged] = useState(() => audibleStart || (bus?.hasFired("mute:unmuted") ?? false));
  ...
  return engaged || locallyEngaged || audibleStart;
}
