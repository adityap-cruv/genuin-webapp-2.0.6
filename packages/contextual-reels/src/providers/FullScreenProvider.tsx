...
function redirectToFallback(videoId?: string): void {
...
export function FullScreenProvider({ children }: FullScreenProviderProps): ReactNode {
  ...
  const enterFullScreen = useCallback(async () => {
    ...
    if (isIOS) {
      if (shouldRedirect) {
        redirectToFallback(videoIdRef.current);
      } else {
        enterManualFullScreen();
      }
      return;
  ...
  // every provider re-render. The enter/exit/toggle callbacks are already
  // useCallback-stable.
  const value = useMemo<FullScreenContextValue>(
    () => ({ isFullScreen, enterFullScreen, exitFullScreen, toggleFullScreen, isRedirectMode }),
    [isFullScreen, enterFullScreen, exitFullScreen, toggleFullScreen, isRedirectMode]
  );
