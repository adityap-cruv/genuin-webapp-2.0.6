...
comment — this doc is the *map*; read the file for the *detail*.
...
| File | Role |
| --- | --- |
| [`src/loader.jsx`](../src/loader.jsx) | Stable, dependency-free bootstrap script partners include (`gen_ext.min.js`). Build placeholders (`__CR_CORE_FILENAME__`, `__CR_CSS_FILENAME__`, `__CR_CDN_BASE__`, `__CR_PIXEL_URL__`) are hash-substituted at build. Captures its own `<script src>` query string into `window.__CXR_SCRIPT_PARAMS__` (the reliable config channel inside a cross-origin `srcdoc` iframe), injects the CSS `<link>`, then dynamically `import()`s the core bundle. On core-load failure it fires a `px-script-error` pixel directly (PixelReporter lives inside the not-yet-loaded core, so its URL shape is duplicated here). |
