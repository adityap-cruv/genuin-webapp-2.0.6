...
| [`PlayerProvider`](../src/providers/PlayerProvider.tsx)         | **`volume` is the source of truth; `isMuted = volume === 0`.** Also `isPlaying`, `isAdBreakActive`. `usePlayer()`: getters + `setVolume/setMuted/setPlaying/setAdBreakActive/notifyAutoplayBlocked`. `DEFAULT_UNMUTE_VOLUME = 0.1`.                                                                                                      |
