...
| `app_loc`, `app_lat`, `app_long`, `app_metro`, `app_region`                                                                     | String         | Host-provided geo from script params (`loc`/`loclat`/`loclong`/`m`/`r`). `app_metro` = Nielsen DMA code (e.g. `505`); `app_region` = state code (e.g. `MI`). Distinct from the IP-based `geoip_*` fields (`geoip_region` is a full name like `Georgia`). |
| `device_type`                                                                                                                   | String         | e.g. `mobile`                                                                                                                                                                                                                                            |
| `user_agent`                                                                                                                    | String         |                                                                                                                                                                                                                                                          |
| `geoip_country_code`, `geoip_country_en`, `geoip_city_en`, `geoip_region`, `geoip_postal`, `geoip_ip`, `geoip_lat`, `geoip_lng` | String / Float | IP-based (from `/ip_info`), never host-provided geo. `region` = state/region name, `postal` = ZIP. `country_en` is often `''` when the gateway sends only `country`.                                                                                     |
