...
| `app_loc`, `app_lat`, `app_long`, `app_metro`                                                                                   | String         | Host-provided geo from script params (`loc`/`loclat`/`loclong`/`m`). `app_metro` = Nielsen DMA code (e.g. `505`). Distinct from the IP-based `geoip_*` fields.       |
