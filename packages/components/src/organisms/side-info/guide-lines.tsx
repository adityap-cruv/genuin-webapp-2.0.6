...
export function Guidelines({
  ...
  const displayedGuidelines = showAll ? guidelines : guidelines.slice(0, INITIAL_DISPLAY_COUNT);
    ...
    <div className={cn(className)} {...restProps}>
      ...
      <Accordion collapsible type="single" className="">
        {displayedGuidelines.map((guideline, index) => (
          <AccordionItem key={guideline.id} value={`guideline-${guideline.id}`} className="gencl:px-1">
            ...
            <AccordionContent className="gencl:pb-2">
              <p className="gencl:text-body-1-semi-bold gencl:text-secondary-500 gencl:ml-4">{guideline.description}</p>
            </AccordionContent>
          </AccordionItem>
        ))}
        {shouldShowMore && (
