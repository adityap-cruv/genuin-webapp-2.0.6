...
export function OwnerInfo({ owner, createdAt }: { owner: PostDetailsType["owner"]; createdAt: string }) {
  if (!owner) return null;

  return (
    <div className="gencl:flex gencl:gap-2 gencl:items-center">
      <Avatar alt={owner.name ?? ""} imageUrl={owner.profileImage} isAvatar={owner.isAvatar} size="md" />
      <ProfileLink
        url={buildPageUrl({
      ...
      <span className="gencl:text-body-1-medium gencl:text-secondary-600">{createdAt}</span>
    </div>
  );
}
