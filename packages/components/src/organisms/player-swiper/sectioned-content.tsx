          ...
          {({ isActive: isHorizontalActive }) => (
            <SwiperImplementation
              ...
              onReachEnd={(swiper: Swiper) => {
                ...
                // recalculates geometry — at that moment activeIndex can transiently read as
                // the last index, so the old `activeIndex < length - 1` guard let it through
                // and flipped isEndOfFeedReached=true on first paint. That hid the iHeart
                // PlayerHeader (back button) until a vertical swipe reset it via onSlideChange.
                // You cannot genuinely reach the end of a multi-slide feed while still on the
                // first slide, so also require activeIndex > 0 (and >1 slide overall).
