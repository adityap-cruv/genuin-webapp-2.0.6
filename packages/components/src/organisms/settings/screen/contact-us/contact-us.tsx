...
export const ContactUs = ({ email }: { email: string }) => {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="gencl:relative gencl:h-full gencl:w-full">
        {isDesktop && <h4 className="gencl:text-headline-4-medium gencl:mb-3">Contact Us</h4>}
        ...
        <FormField
          ...
          render={({ field, fieldState }) => (
            <FormItem className="gencl:sm:w-full gencl:mb-4 gencl:mt-2">
              <FormLabel className="is-required">Describe your issue</FormLabel>
              <FormControl>
                <Textarea
        ...
        <div className="gencl:flex gencl:justify-end">
          <Button variant="default" type="submit" disabled={!isValid || !isDirty || contactUsMutation.isPending}>
            <SendIcon />
            <p className="gencl:text-new-para-2 gencl:text-tertiary-100">Send</p>
          </Button>
        </div>
      </form>
    </Form>
