...
const VARIANT_PADDING: Record<"default" | "compact" | "expanded", string> = {
  default: "gencl:p-8 gencl:mt-8 gencl:sm:m-0! gencl:sm:p-12!",
  compact: "gencl:p-8",
  expanded: "gencl:p-8",
};
...
export function ModalShell({ children, className, showBack, variant = "default", ...restProps }: ModalShellProps) {
  const { step, setStep } = useAuthenticationModalContext();
  const prevStep = usePrevious(step);

  return (
    <div className={cn("gencl:relative gencl:space-y-6", VARIANT_PADDING[variant], className)} {...restProps}>
      {showBack && prevStep && (
        <Button
          variant="icon"
          theme="text"
