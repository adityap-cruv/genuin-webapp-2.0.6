...
const formSchema = z.object({
  url: z.string().nonempty("URL is required").refine(isValidUrlFormat, {
    message: "Invalid URL",
  }),
  title: z.string().trim().max(56, { message: "Max length should be 56" }),
});
...
export const AddLinks: React.FC<AddLinksProps> = ({
  ...
  const { mutate: fetchMetaPost, isPending: fetchMetaData } = usePostFetchMetaMutation({
    onSuccess: async ({ res }) => {
    ...
    onError: () => {
      form.setError("root", {
        message: "Something went wrong while fetching meta data. Please try again.",
      });
    },
  });
  ...
  const convertImageUrlToBlob = async (imageUrl: string) => {
    ...
    try {
      ...
      const uploadRes = await uploadProfileImage(file, UPLOAD_LINKOUTS_PATH, axiosInstance);
      if (uploadRes) {
    ...
    <Form {...form}>
      <form
        ...
        <LinkoutLinkInput
          ...
          showFieldIcon
          callbackFunc={(url: string) => fetchMetaPost({ url })}
        />
            ...
            return (
              <FormItem className="gencl:sm:w-full gencl:mb-4 gencl:mt-2">
                <FormLabel className="gencl:flex gencl:justify-between">
                  Link title
                  <span className="gencl:text-secondary-500 gencl:text-body-2-medium">{currentLength}/56</span>
                </FormLabel>
                <FormControl>
                  <Input
        ...
        <LinkThumbnail
          ...
          width={100}
          onImageChange={({ file }) => {
            setFileUrl(`${process.env.NEXT_PUBLIC_MEDIA_BASE_URL}/uploads/linkouts/${file.name}`);
        ...
        <div className="gencl:flex gencl:justify-end gencl:gap-3">
          ...
          <Button size="sm" variant="default" type="submit" disabled={!isValid || (!isDirty && !thumbnailRemoved)}>
            Save
          </Button>
        </div>
      </form>
