...
const ProfileDetailsSchema = z.object({
  ...
  no_of_groups: z.number().default(0),
  videos: z.number().default(0),
  views: z.string().or(z.number()).default("0"),
});

/**
 *
 * @param data - The data to validate against the profile details schema.
 * @returns
 */
export function validateProfileDetails(data: unknown) {
  try {
