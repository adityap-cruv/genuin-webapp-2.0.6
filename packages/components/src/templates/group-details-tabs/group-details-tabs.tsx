...
export function GroupDetailsTabs({ slug, className, aboutComponent, ownerId, ...restProps }: GroupDetailsTabsProps) {
...
function GroupMembers({ slug, ownerId }: { slug: string; ownerId?: string }) {
  const { isError, isLoading, data: membersData, fetchNextPage, hasNextPage } = useGetGroupMembers(slug);

  const members = useMemo(() => membersData?.pages.flatMap((page) => page.members) ?? [], [membersData]);
