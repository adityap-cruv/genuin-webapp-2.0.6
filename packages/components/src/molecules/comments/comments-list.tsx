...
import { handleReactionStateChangeInComments, useComments } from "@genuin/components/react-query/api/comments";
...
export const CommentsList = memo(function CommentsList({
  ...
  const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage, isError } = useComments(videoId);
  ...
  if (isError) {
    return (
      <div
        ...
        <p className="gencl:text-body-2-medium gencl:text-secondary-300">We’re unable to load comments.</p>
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <div
        className={cn("gencl:w-full gencl:h-full gencl:flex gencl:flex-col gencl:shrink-0 gencl:p-4", className)}
        {...restProps}>
    ...
    return (
      <div
        ...
        <div className="gencl:space-y-1">
          <p className="gencl:text-body-0-semi-bold gencl:text-center">No Comments Yet</p>
          <p className="gencl:text-secondary-600 gencl:text-body-2-medium">Be the first one to comment!</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      className={cn("gencl:h-full gencl:w-full gencl:overflow-auto gencl:p-4 gencl:space-y-4 gencl:!pb-16", className)}
      {...restProps}>
      <InfiniteScroll isLoadingNextPage={isFetchingNextPage} hasNextPage={hasNextPage} getNextPage={fetchNextPage}>
