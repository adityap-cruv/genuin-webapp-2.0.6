...
export function UsernameInput({ onNext }: ScreenProps) {
  ...
  const { user, updateUser: updateAuthUser } = useAuthContext();
  const [isLoading, setIsLoading] = useState(false);
  const { setFormData, formData } = useAuthenticationModal();
  ...
  async function onSubmit({ username }: { username: string }) {
    ...
    try {
      if (isUsernameValid) {
        ...
        if (status) {
          if (user)
            updateAuthUser({ ...user, nickname: username, usernameSet: true });
          // await updateSession({
          //   ...sessionData,
          //   user: {
          //     ...sessionData?.user,
          ...
          // });
          onNext();
          // todo: Address this analytics event
  ...
  return (
    <ModalShell>
      <div>
        ...
        <p className="flex w-full justify-center text-title-3-med text-tertiary">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="w-full">
                  ...
                  <FormLabel className="text-body-1-med">
                    <div className="flex w-full justify-between">
                      ...
                      <p className="text-cap-1-med ">
                        {form.getValues("username")?.length ?? 0}/25
                      </p>
                    </div>
                  </FormLabel>
                  <FormControl>
          ...
          >
            {isLoading ? (
              ...
              <p className="text-title-3-demi text-monochrome-white">
                Save and proceed
              </p>
            )}
          </Button>
