...
import { encryptText } from "@genuin/ui/utils";
import { LOGIN_SOURCE } from "src/lib/constants";
...
export async function uploadProfileImage(file: File) {
  try {
    const getUrlResponse = await axiosInstance.post(
      "/api/v3/users/video/upload/create_upload_url",
      {
...
export async function deleteUserAccount(): Promise<{
  ...
  return await axiosInstance
    ...
    .catch((e) => {
      return {
        code: Number(e?.response?.data.code),
    ...
    .then((res) => {
      if (res.data.code === 200) {
        return parseUserData(
          res.data.data,
          res.headers["gn-access-token"],
          res.headers["gn-refresh-token"]
        );
      }
      return null;
    })
