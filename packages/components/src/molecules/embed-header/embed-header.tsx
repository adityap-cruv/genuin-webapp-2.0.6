...
const embedHeaderVariants = cva("gencl:flex gencl:shrink-0 gencl:gap-2 gencl:w-full", {
  variants: {
    variant: {
...
type EmbedHeaderPropsType = ComponentProps<"div"> & VariantProps<typeof embedHeaderVariants>;

/**
 * This component is used for embed only.
 *  */
export function EmbedHeader({ className, variant, ...restProps }: EmbedHeaderPropsType) {
  ...
  return (
    <div className={cn(embedHeaderVariants({ variant }), "gencl:p-2")} {...restProps}>
      <div>
        ...
        {header.subHeading && (
          <p
            ...
            style={{
              color: header.subHeadingTextColor ? header.subHeadingTextColor : "var(--gencl-secondary-300)",
            }}>
            {header.subHeading}
          </p>
        )}
      ...
      {header.ctaButton?.url && (
        <Link
          style={{
            color: header.ctaButton.textColor ? header.ctaButton.textColor : "white",
            backgroundColor: header.ctaButton.color ? header.ctaButton.color : "var(--gencl-primary-500)",
          }}
          onClick={() => {
