...
export type AnchorTagType = UrlAnchorTag | MemberAnchorTag | CommunityAnchorTag | CustomAnchorTag | string | null;

// Utility function to convert URLs, mentions, and slugs to anchor tag data objects (for rendering in React)
export function convertUrlsToAnchorTags(strArr: Array<Record<string, unknown> | string | null>): AnchorTagType[] {
  return strArr.map((item) => {
    if (typeof item === "object" && item !== null && "url" in item && "text" in item) {
      return { type: "url", url: String(item.url), text: String(item.text) };
    }
    if (typeof item === "object" && item !== null && "member_id" in item && "text" in item) {
      return {
        type: "member",
    ...
    if (typeof item === "object" && item !== null && "slug" in item && "text" in item) {
      return {
        type: "community",
    ...
    if (typeof item === "object" && item !== null && "text" in item && "style" in item) {
      return {
        type: "custom",
 ...
 */
export function renderAnchorTag(item: AnchorTagType, index: number): React.ReactNode {
  ...
  switch (item.type) {
    case "url": {
      const href = item.url.startsWith("http") ? item.url : `https://${item.url.replace(/^\/+/g, "")}`;
  ...
  try {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      if (fitsInLines(testString)) {
        ...
        const previousString = resultString;

        for (let charIndex = 0; charIndex < word.length; charIndex++) {
          const char = word[charIndex];
          const testStringWithChar = charIndex === 0 && i > 0 ? `${previousString}${char}` : `${resultString}${char}`;
 ...
 */
export function calculateMaxHeight(element: HTMLElement | null, maxLines: number): string {
  ...
  if (lineHeightValue.endsWith("px")) {
    ...
    const lineHeightMultiplier = lineHeightValue === "normal" ? 1.2 : parseFloat(lineHeightValue);
