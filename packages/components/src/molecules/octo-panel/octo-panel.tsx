...
export const OctoPanel = forwardRef<OctoPanelHandle, OctoPanelPropsType>(function OctoPanel(
  ...
  const { onClick: triggerOnClick, role: triggerRole, tabIndex: triggerTabIndex, ...restTriggerProps } = triggerProps;
  ...
  if (variant === "sheet") {
    return isOpen ? (
      <div className={cn("gencl:flex-1 gencl:min-h-0", panelClassName)}>{sdkContainerContent}</div>
    ) : null;
  }
      ...
      {isOpen ? (
        <div
          ...
          <div className="gencl:flex gencl:justify-end gencl:px-4 gencl:pt-4 gencl:pb-0">
            <Button theme="custom" variant="icon" className="gencl:size-5!" onClick={handleClose}>
              <X className="gencl:size-5 gencl:cursor-pointer gencl:opacity-70 gencl:transition-opacity gencl:hover:opacity-100" />
            </Button>
          </div>

          <div className="gencl:flex-1 gencl:min-h-0">{sdkContainerContent}</div>
        </div>
      ) : null}
    </>
  );
