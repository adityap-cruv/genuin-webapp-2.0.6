import { MuteIconOld } from "@genuin/ui/icons";
import { UnmuteIconOld } from "@genuin/ui/icons";
...
export function PlayingState({ className, showOnlyPlayAction = false, ...restProps }: PlayingStateProps) {
    ...
    return (
      <div
        ...
        <div
          ...
          {...restProps}>
          <PauseIconOld theme="dark" size="xl" aria-hidden="true" />
        </div>
  ...
  if (showOnlyPlayAction) {
    // Only show PlayIconOld when action is PLAY, hide all other actions (PAUSE, MUTE, UNMUTE)
    if (buttonAction === "PAUSE") {
      return (
        <div
          ...
          {...restProps}>
          <PlayIconOld theme="fill-dark" size="xl" aria-hidden="true" />
        </div>
      );
    }
  ...
  const renderIcon = () => {
    switch (buttonAction) {
      case "PLAY":
        return <PlayIconOld theme="fill-dark" size="xl" aria-hidden="true" />;
      case "MUTE":
        return <MuteIconOld theme="dark" size="xl" aria-hidden="true" />;
      case "UNMUTE":
        return <UnmuteIconOld theme="dark" size="xl" aria-hidden="true" />;
      default:
        return null;
    }
  };
