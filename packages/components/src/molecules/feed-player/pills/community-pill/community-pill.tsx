  ...
  "gencl:flex gencl:w-fit gencl:items-center gencl:gap-1 gencl:p-1 gencl:pr-2 gencl:rounded-full gencl:transition-all gencl:cursor-pointer",
  {
    variants: {
      variant: {
        ...
        compact: "gencl:bg-secondary-50! gencl:text-default gencl:border-none gencl:hover:bg-secondary-150!",
      },
    },
    defaultVariants: {
      variant: "light",
...
export function CommunityPill({
  ...
  const [localJoinStatus, setLocalJoinStatus] = useState(communityDetails?.userRole);
  const prevJoinStatus = usePrevious(communityDetails?.userRole);

  // Truncate name to 24 characters for compact variant
  const displayName =
    variant === "compact" && communityDetails?.name && communityDetails?.name.length > 24
  ...
  }, [communityDetails?.userRole, prevJoinStatus]);

  const hideButton = localJoinStatus === "MEMBER" || authenticationStatus === "unauthenticated";
      ...
      <div className={communityPillVariants({ variant, className })}>
        <div className="gencl:flex gencl:gap-1 gencl:items-center gencl:line-clamp-1 gencl:break-all">
          ...
          <span
            ...
            style={embedDetails?.embedData.card_layout_id === 3 ? { lineHeight: "18.2px" } : {}}>
            {displayName}
          </span>
        </div>
        {authenticationStatus === "authenticated" && variant !== "compact" && !hideCommunityJoinButton && (
          <div
