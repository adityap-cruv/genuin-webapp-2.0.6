...
import type { JoinGroupButton } from "@genuin/components/molecules/join-group-button";
...
export function GroupPill({
  ...
  const [localSubscriptionStatus, setLocalSubscriptionStatus] = useState<undefined | boolean>(undefined);
   ...
   */
  useEffect(() => {
    if (prevSubscriptionStatus.current === false && groupDetails?.isSubscribed === true) {
    ...
    <Link href={buildPageUrl({ type: "group", slug: groupDetails.slug })}>
      <div className={groupPillVariants({ variant, className })}>
        <div className="gencl:flex gencl:gap-1 gencl:items-center gencl:line-clamp-1 gencl:break-all">
          ...
          <span className="gencl:text-body-2-medium gencl:line-clamp-1">{displayName}</span>
        </div>
        {authenticationStatus === "authenticated" && variant !== "compact" && !hideGroupSubscriptionButton && (
          <div
