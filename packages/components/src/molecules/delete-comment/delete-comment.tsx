...
import { deleteCommentFromQueryData, useDeleteCommentMutation } from "@genuin/components/react-query/api/comments";
    ...
    <Dialog modal open={isOpen} onOpenChange={handleOpenChange} {...props}>
      ...
      <DialogContent className="gencl:max-w-xl gencl:rounded-t-2xl! gencl:md:rounded-2xl! gencl:flex gencl:flex-col gencl:gap-4 gencl:sm:gap-5!">
        <DialogHeader className="gencl:border-none gencl:text-center gencl:sm:text-start! gencl:gap-3 gencl:sm:gap-6!">
          <p className="gencl:text-headline-3-semi-bold gencl:text-black">Delete comment?</p>
          <p className="gencl:text-body-1-medium gencl:sm:text-body-0-medium! gencl:text-secondary-500 gencl:sm:text-black!">
            Are you sure you want to delete this comment? You won’t be able to get this comment back.
          </p>
        </DialogHeader>
        ...
        <div className="gencl:flex gencl:flex-col-reverse gencl:sm:flex-row! gencl:justify-end gencl:gap-2">
          ...
          <Button
            ...
            disabled={deleteCommentMutation.isPending}>
            {deleteCommentMutation.isPending ? <Loader className="gencl:size-4" /> : "Delete"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
