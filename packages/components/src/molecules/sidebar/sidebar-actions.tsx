...
const sidebarActionsVariants = cva("gencl:px-3 gencl:py-4 gencl:!w-full gencl:border-b gencl:border-secondary-100", {
  variants: {
    variant: {
    ...
    <div className={cn(className, sidebarActionsVariants({ variant }))} {...restProps}>
      {SideBarActionLinks.map((links, index) => {
        ...
        if (links.type === "search") {
          return showSearch ? (
            <SearchSidebarAction key={index} index={index} links={links} onItemClick={onItemClick} />
          ) : null;
        }
        // For posts menu, don't show on mobile devices or if camera_enabled is false or if creatorAccessEnabled is false
      ...
      <Popover>
        ...
        <PopoverContent
          ...
          <Link
            href={brandConfiguredTerms ? brandConfiguredTerms : NEXT_PUBLIC_HOST_URL + "/terms"}
            onClick={onItemClick}>
          ...
          <Link
            href={brandConfiguredPrivacy ? brandConfiguredPrivacy : NEXT_PUBLIC_HOST_URL + "/privacy"}
            onClick={onItemClick}>
            <p className="gencl:text-body-1-medium gencl:p-2 gencl:hover:bg-secondary-50 gencl:cursor-pointer gencl:rounded-lg">
              Privacy Policy
            </p>
    ...
    <div
      ...
      <div className="gencl:w-6 gencl:h-6 gencl:relative">
        {type === "profile" && user ? (
          ...
          Icon && <Icon className="gencl:w-6 gencl:h-6" variant={isActive ? "active" : "default"} />
        )}
        {type === "notification" && notificationCount && notificationCount > 0 && (
