...
const AgentCards = ({ agents }: { agents: Agent[] }) => {
    ...
    return agents
        .filter(agent => agent.type !== 'octo_head')
        .map(agent => (
            <div
                key={agent.id}
            ...
            >
                ...
                <div className='gai:flex gai:h-full gai:min-w-[330px] gai:flex-col gai:items-start gai:gap-3 gai:rounded-[15px] gai:bg-utility-white gai:px-4 gai:py-3'>
                    ...
                    {/* description */}
                    <p className='gai:line-clamp-4 gai:flex-1 gai:self-stretch gai:font-body-2-med gai:text-secondary-gray-700 gai:md:font-body-1-med'>
                        {agent.description}
                    </p>
                </div>
            </div>
