...
const api = axios.create({
    // baseURL: import.meta.env.VITE_GENAI_API_URL + '/api/v1/ds/agents',
    // baseURL: import.meta.env.VITE_GENAI_API_URL + '/agents',
    baseURL: import.meta.env.VITE_GENAI_API_URL,
    headers: {
...
const bccApi = axios.create({
    baseURL: import.meta.env.VITE_GENAI_BCC_API_URL,
});

const dsBackendApi = axios.create({
    baseURL: import.meta.env.VITE_GENAI_DS_BACKEND_API_URL,
});
...
export async function updateBrandAssets(payload: BrandAssetsJsonData) {
    ...
    if (payload.brand_slogan?.slogan) {
        preparedJsonData.slogan = {
            ...
            font: {
                id: import.meta.env.VITE_GENAI_FONT_ID,
                style: 'normal',
                weight: 'lighter',
            },
        };
...
export function getChatStreamUrl(sessionId: string): string {
    return `${import.meta.env.VITE_GENAI_API_URL}${routes.chatStream}/${sessionId}`;
}
