...
const showCustomToast = (
  ...
  toast.custom((id) => (
    <div
      ...
      tabIndex={-1}>
      <div className={cn("gencl:flex gencl:items-center gencl:gap-2", description && "gencl:items-start")}>
        {variant === "success" ? (
          <div
            className={cn(
              "gencl:size-5 gencl:rounded-2xl gencl:flex gencl:justify-center gencl:items-center gencl:shrink-0",
              isIheart ? "gencl:bg-[#46815A]!" : "gencl:bg-success-status gencl:border gencl:border-white"
            )}>
            <IHeartCheckIcon
              className={cn("gencl:shrink-0 gencl:fill-white", isIheart ? "gencl:size-4" : "gencl:size-3")}
            />
          </div>
        ) : (
        ...
        <div className="gencl:text-body-0-semi-bold gencl:text-secondary-900 gencl:flex gencl:flex-col gencl:gap-2">
          ...
          {description && <p className="gencl:text-body-1-medium gencl:m-0!">{description}</p>}
        </div>
      </div>
      <XIcon
        onClick={(event) => {
