...
const showCustomToast = (message: string, description : string | undefined, variant: ToastVariant) => {
  toast.custom((id) => (
    <div className="gencl:bg-white gencl:text-black gencl:flex gencl:items-center gencl:justify-between gencl:gap-4 gencl:p-4 gencl:rounded-lg gencl:shadow-lg gencl:w-sm gencl:max-w-sm">
      <div className="gencl:flex gencl:items-baseline gencl:gap-2">
        {variant === "success" ? (
...
const ToastSuccess = ({ message , description }: ToastBodyProps) => showCustomToast(message, description , "success");

const ToastError = ({ message , description }: ToastBodyProps) => showCustomToast(message, description, "error");
