...
/**
 * Draggable bottom-sheet with three render strategies:
 * - "fixed" (default): portal to `document.body`, `position:fixed`, full viewport.
 * - "container": `position:absolute` in nearest positioned ancestor; slides via
 *   `translateY`. Pass `containerRef` for height measurement.
 * - "inline": no positioning — parent controls placement; height animates `0 → computed`.
 *   `containerRef` (optional) gives %-based heights, else it measures its own parent.
 *   The `containerRef` may resolve after the first commit (e.g. set in a consumer's
 *   layout effect); the sheet rebinds to it as soon as it populates.
 *
 * `isOpen` drives the animation; `onDismissed` fires after close so the parent can unmount.
 */
function DynamicSheet({
  ...
  // Inline-mode fallback when no containerRef: walk to this panel's parentElement.
  ...
  // States configured with `height: "auto"` resolve to a measured pixel value at
  // runtime (sum of indicator + header + content + footer) that flows through
  // `heights` into the snap math, so drag/snap/animation all work with auto
  // sizing — and the panel grows or shrinks in real time as the content changes
  // (e.g. title wrapping to one vs two lines).
  const indicatorRef = useRef<HTMLDivElement>(null);
  const headerWrapRef = useRef<HTMLDivElement>(null);
  const footerWrapRef = useRef<HTMLDivElement>(null);
  // Separate ref on the same node as `contentRef` (below) so the measurement
  // effect can read content height independently of the scroll logic.
  ...
  // `autoHeightProvider`. Lets the engine resolve a *distinct* height for each
  // enabled `"auto"` state even when their bodies differ from the
  // currently-mounted `children`. Without this map, two adjacent `"auto"` states
  // would collide on the active body's measurement (the bug `embed-xs` /
  // `embed-sml` previously worked around with `expand-view: "50vh"`, which
  // over-sized the panel).
  ...
  // List of states whose configured height is `"auto"` and for which the host
  // provides a body renderer. The wells are only mounted when both hold.
  ...
  const resolvedHeights = useMemo(() => {
    ...
    for (const state of DYNAMIC_SHEET_STATES) {
      ...
      if (value !== undefined && isAutoHeight(value)) {
        // Prefer the per-state measurement when available; fall back to the
        // shared active-body measurement (legacy path).
  ...
  const { currentState, isDragging, transientHeightPx, handleDragStart, stateToPx, transitionTo } = useDynamicSheet({
    ...
    stepByStepSwipeUp: config.stepByStepSwipeUp ?? false,
    disableDragAndSwipe: config.disableDragAndSwipe ?? false,
    commitOnDragEnd: config.commitOnDragEnd ?? false,
    autoAdvance: config.autoAdvance,
  });

  // Auto-height measurement: re-runs whenever the rendered content changes shape
  // — when the host swaps `header` / `footer` / `children`, AND when the sheet
  // transitions between states. Reads the dedicated inner wrapper
  // (`measureContentRef`), not the scrollable container — `scrollHeight` on the
  // scrollable parent collapses to its `clientHeight` when content fits, which
  // would freeze the panel at its previous size. Skipped entirely for
  // fixed-height states (`100%`, `70vh`, `200px`, …) since the panel uses that
  // value directly via `resolvedHeights`, avoiding needless DOM reads.
  useEffect(() => {
  ...
  // Per-state auto measurement (no-op without `autoHeightProvider`). Each well
  // renders the full panel-shaped subtree (header + body + footer), so its
  // `offsetHeight` already accounts for that state's chrome — we must NOT add
  // `headerH` / `footerH` again here or we'd double-count chrome and resolve
  // oversized panels. The indicator strip lives outside the chrome wrapper, so
  // it's still added.
  useEffect(() => {
    ...
    const recompute = () => {
      ...
      // Skip until a well measures non-zero, else first paint flips to a
      // chrome-only height and briefly collapses the panel.
    ...
    const observer = new ResizeObserver(recompute);
    // Observe every well plus shared chrome refs so chrome changes (e.g. header
    // wrapping) update every state's resolved height together.
  ...
  // Fixed-height states give the measurement wrapper `height: 100%` so children
  // can fill the panel (else inner `h-full` resolves circularly against the
  // wrapper's auto height). Auto states keep it auto so `offsetHeight` reads
  ...
  // Also re-transition while open if `config.initialState` changes — hosts (e.g.
  // parent React state machines) can use this to push a new "starting state"
  // after mount, which is otherwise impossible because `useDynamicSheet`'s
  // `useState` only takes the initial value on first render.
  ...
  const handleContentPointerDown = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      // Stop propagation so the sheet's handleDragStart never fires from inside
      ...
      if (el.scrollTop === 0 && e.button === 0) {
        // At the top — track for a potential pull-to-close.
  ...
  const handleContentPointerMove = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      // Content scrolled since gesture start → user is scrolling, not pulling.
      if (el.scrollTop > 0) {
  ...
  // `"auto"` / `"fit-content"` heights are resolved to a measured pixel value
  // (see `resolvedHeights` above), so by the time we get here every height is a
  // real number. Snap math and CSS just deal with pixels.
  ...
  // In "fixed"/"container": always show snappedHeight, use translateY for the
      ...
      <div
        ...
        {header ? (
          ...
          <div
            ...
            className={cn(
              ...
              // Cursor lies about draggability if we always set `cursor-grab` —
              // single-state scenarios pass `disableDragAndSwipe: true` and have
              // no pointer handler attached, so the grab affordance shouldn't appear.
        ...
        {children && (
          <div
            ...
            {/* Inner measurement wrapper: takes its natural height from the
                rendered children (no flex / overflow), so its `offsetHeight`
                reflects the actual content size. Reading scrollHeight on the
                scrollable parent returns the parent's clientHeight when content
        ...
        {/* Hidden per-state measurement wells. Mounted only when the host
            supplies `autoHeightProvider` and the state's configured height is
            `"auto"`. The wrapper is taken out of layout (`position: absolute`,
            `visibility: hidden`, `pointer-events: none`) and marked inert /
            aria-hidden so assistive tech, focus, and pointer events all skip it.
            Width matches the panel so the well's `offsetHeight` reflects the
            body's actual rendered height at the panel's own width. */}
        {autoHeightProvider && autoStates.length > 0 && (
          <div
            aria-hidden="true"
            // `inert` is a valid HTMLAttribute in React 19 — opts every
            // descendant out of tab order, focus, and the accessibility tree.
            inert
            ...
            {autoStates.map((state) => (
              <div
                ...
                // `contain: layout` isolates each well's internal layout
                // calculations from the rest of the panel subtree. Without this,
                // a hidden well that mounts its own widget (e.g. a Swiper
                // instance, or any ResizeObserver-driven layout) inside a
                // 0-height container can pollute shared rendering state — the
                // observed regression was `embed-default`'s panel-view thumb
                // collapsing to 0×0 because the extra `default` well's body
                // shared a layout context with the visible body's Swiper slide chain.
