...
const meta: Meta<typeof VideoPlayer> = {
  ...
  argTypes: {
    ...
    controls: {
      control: "boolean",
      description: "Whether to display controls (derived from OpenPlayerJS config). Default: true (visible)",
    },
    width: {
      control: "text",
    ...
    onOpenPlayerReady: {
      ...
      description: "Callback when player is ready (OpenPlayerJS specific, maps to onOpenPlayerReady).",
    },
    onPlay: {
      action: "onPlay",
...
const EventHandlingPlayer = (props: PlayerProps) => {
  ...
  const handleEvent = useCallback(
    (eventName: string, callback?: (...args: unknown[]) => void) => {
      return (...args: unknown[]) => {
        setLastEvent(`${eventName} fired at ${new Date().toLocaleTimeString()}`);
        if (callback) {
  ...
  return (
    <div>
      <VideoPlayer
        ...
        onOpenPlayerReady={handleEvent("onOpenPlayerReady", props.onOpenPlayerReady as (...args: unknown[]) => void)}
        onPlay={handleEvent("onPlay", props.onPlay as (...args: unknown[]) => void)}
        onPause={handleEvent("onPause", props.onPause as (...args: unknown[]) => void)}
        onEnded={handleEvent("onEnded", props.onEnded as (...args: unknown[]) => void)}
        onError={handleEvent("onError", props.onError as (...args: unknown[]) => void)}
        onTimeUpdate={handleEvent("onTimeUpdate", props.onTimeUpdate as (...args: unknown[]) => void)}
        onVolumeChange={handleEvent("onVolumeChange", props.onVolumeChange as (...args: unknown[]) => void)}
        onPlayerLoad={handleEvent("onPlayerLoad", props.onPlayerLoad as (...args: unknown[]) => void)}
      />
      <p style={{ marginTop: "10px", fontFamily: "monospace" }}>Last Event: {lastEvent}</p>
    </div>
  );
};
