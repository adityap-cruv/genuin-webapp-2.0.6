...
function TabsSkeleton({ noOfTabs = 2, className }: { noOfTabs?: number; className?: string }) {
  return (
    <div className={cn("gencl:border-b gencl:border-b-secondary-200", className)}>
      <div className="gencl:flex gencl:items-center gencl:gap-3 gencl:py-2 gencl:ps-2">
        {Array.from({ length: noOfTabs }).map((_, idx) => (
          <Skeleton key={idx} className="gencl:w-13 gencl:h-5 gencl:rounded-md gencl:shrink-0" />
        ))}
      </div>
    </div>
  );
