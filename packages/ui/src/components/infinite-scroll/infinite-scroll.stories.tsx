  ...
  render: function DefaultInfiniteScroll() {
    ...
    return (
      <div style={{ height: "500px", overflowY: "auto", border: "1px solid #ccc" }}>
        <InfiniteScroll hasNextPage={hasNextPage} isLoadingNextPage={isLoadingNextPage} getNextPage={getNextPage}>
          {items.map((item) => (
...
export const NoMorePages: Story = {
  ...
  render: (args) => (
    <div style={{ height: "300px", overflowY: "auto", border: "1px solid #ccc" }}>
      ...
      {!args.hasNextPage && <div style={{ textAlign: "center", padding: "20px", color: "#888" }}>End of content.</div>}
    </div>
  ),
};
        ...
        <InfiniteScroll
          ...
          loader={
            <div style={{ padding: "20px", textAlign: "center", color: "blue" }}>
              Loading more items, please wait... <Loader size="sm" className="ml-2 inline-block" />
            </div>
          }>
          {items.map((item) => (
            <ItemComponent key={item.id} text={item.text} />
