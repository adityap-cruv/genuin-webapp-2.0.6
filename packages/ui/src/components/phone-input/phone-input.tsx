...
const InputComponent = React.forwardRef<HTMLInputElement, InputProps>(({ className, ...props }, ref) => (
  <Input
    className={cn("gencl:ml-2 gencl:rounded-lg gencl:border gencl:border-secondary-150", className)}
    {...props}
    ref={ref}
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          type="button"
          theme="outline"
          className={cn("gencl:flex gencl:gap-1 gencl:rounded-lg gencl:border-secondary-150 gencl:pl-3 gencl:pr-1")}
          disabled={disabled}>
          <FlagComponent country={value} countryName={value} />
          <ChevronsUpDown
            className={cn("gencl:h-4 gencl:w-4 gencl:opacity-50", disabled ? "gencl:hidden" : "gencl:opacity-100")}
          />
        </Button>
      </PopoverTrigger>
      <PopoverContent className={cn("gencl:w-[300px] gencl:z-50 gencl:p-0", popoverClassName)}>
              ...
              {options
                ...
                .map((option) => (
                  <CommandItem
                    ...
                    <span className="gencl:text-body-1-semi-bold gencl:flex-1 gencl:text-left">{option.label}</span>
                    {option.value && (
