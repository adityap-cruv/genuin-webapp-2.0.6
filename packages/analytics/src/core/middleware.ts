...
export class MiddlewareChain {
  ...
  remove(middleware: Middleware): void {
    this.middlewareList = this.middlewareList.filter((entry) => entry.middleware !== middleware);
  }
  ...
  async execute(event: AnalyticsEvent): Promise<void> {
    ...
    const enabledMiddleware = this.middlewareList.filter((entry) => entry.options.enabled);
